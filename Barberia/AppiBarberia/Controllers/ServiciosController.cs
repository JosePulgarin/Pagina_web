using LibreriaBarberia.Entidades;
using LibreriaBarberia.Implementaciones;
using LibreriaBarberia.Interfaces;
using LibreriaBarberia.Nucleo;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;

namespace AppiBarberia.Controllers // Asegúrate de que coincida con el nombre de tu proyecto API
{
    [ApiController]
    // Usamos el ruteo estándar de REST, la URL será: http://localhost:puerto/Servicios
    [Route("[controller]")]
    public class ServiciosController : ControllerBase
    {
        // 1. El contrato con el Chef
        private IServiciosNegocio? iServiciosNegocio;

        // 2. El Constructor: Aquí contratamos al Chef
        public ServiciosController()
        {
            this.iServiciosNegocio = new ServiciosNegocio();
        }

        // ==========================================
        // C - CREATE (Guardar)
        // ==========================================
        [HttpPost]
        public Servicios Guardar(Servicios entidad)
        {
            if (this.iServiciosNegocio == null)
                throw new Exception("No implementado");

            // El Mesero le pasa el pedido al Chef
            return this.iServiciosNegocio!.Guardar(entidad);
        }

        // ==========================================
        // R - READ (Consultar Todos)
        // ==========================================
        [HttpGet]
        public List<Servicios> Consultar()
        {

            if (this.iServiciosNegocio == null)
                throw new Exception("No implementado");

            // El Mesero trae la lista que hizo el Chef
            return this.iServiciosNegocio!.Consultar();
        }

        // ==========================================
        // R - READ (Consultar por Id)
        // ==========================================
        [HttpGet("{id}")] // Le decimos a Swagger que espere un número en la URL
        public Servicios ConsultarPorId(int id)
        {
            if (this.iServiciosNegocio == null)
                throw new Exception("No implementado");

            return this.iServiciosNegocio.ConsultarPorId(id);
        }

        // ==========================================
        // U - UPDATE (Modificar)
        // ==========================================
        [HttpPut]
        public Servicios Actualizar(Servicios entidad)
        {
            if (this.iServiciosNegocio == null)
                throw new Exception("No implementado");

            return this.iServiciosNegocio!.Actualizar(entidad);
        }

        // ==========================================
        // D - DELETE (Eliminar)
        // ==========================================
        [HttpDelete("{id}")] // Le decimos a Swagger qué número borrar
        public bool Eliminar(int id)
        {
            if (this.iServiciosNegocio == null)
                throw new Exception("No implementado");

            return this.iServiciosNegocio!.Eliminar(id);
        }
    }
}