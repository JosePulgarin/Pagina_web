...
{
    public class RecepcionistasService
    {
        private readonly IComunicaciones _puente;

        // La URL base de tu API (Asegúrate de poner el puerto correcto que te da Swagger)
        private readonly string _urlApi = "http://localhost:5247/Recepcionistas";

        // El constructor recibe el puente mágicamente
        public RecepcionistasService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<RecepcionistasClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<RecepcionistasClase>>(json!) ?? new List<RecepcionistasClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<RecepcionistasClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /Recepcionistas/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<RecepcionistasClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(RecepcionistasClase Recepcionista)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", Recepcionista } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(RecepcionistasClase Recepcionista)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", Recepcionista } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
