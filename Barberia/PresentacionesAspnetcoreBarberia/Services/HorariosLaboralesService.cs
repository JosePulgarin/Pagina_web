...
{
    public class HorariosLaboralesService
    {
        private readonly IComunicaciones _puente;

        // La URL base de tu API (Asegúrate de poner el puerto correcto que te da Swagger)
        private readonly string _urlApi = "http://localhost:5247/HorariosLaborales";

        // El constructor recibe el puente mágicamente
        public HorariosLaboralesService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<HorariosLaboralesClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<HorariosLaboralesClase>>(json!) ?? new List<HorariosLaboralesClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<HorariosLaboralesClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /HorariosLaborales/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<HorariosLaboralesClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(HorariosLaboralesClase HorarioLaboral)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", HorarioLaboral } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(HorariosLaboralesClase HorarioLaboral)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", HorarioLaboral } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
