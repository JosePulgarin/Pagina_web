...
namespace PresentacionesAspnetcoreBarberia.Services
{
    public class MembresiasService
    {
        ...
        private readonly string _urlApi = "http://localhost:5247/Membresias";

        // El constructor recibe el puente mágicamente
        public MembresiasService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<MembresiasClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<MembresiasClase>>(json!) ?? new List<MembresiasClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<MembresiasClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /Membresias/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<MembresiasClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(MembresiasClase Membresia)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", Membresia } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(MembresiasClase Membresia)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", Membresia } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
