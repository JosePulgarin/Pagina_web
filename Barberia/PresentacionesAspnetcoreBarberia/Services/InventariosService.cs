...
namespace PresentacionesAspnetcoreBarberia.Services
{
    public class InventariosService
    {
        ...
        private readonly string _urlApi = "http://localhost:5247/Inventarios";

        // El constructor recibe el puente mágicamente
        public InventariosService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<InventariosClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<InventariosClase>>(json!) ?? new List<InventariosClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<InventariosClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /Inventarios/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<InventariosClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(InventariosClase Inventario)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", Inventario } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(InventariosClase Inventario)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", Inventario } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
