...
{
    public class ReseñasClientesService
    {
        private readonly IComunicaciones _puente;

        // La URL base de tu API (Asegúrate de poner el puerto correcto que te da Swagger)
        private readonly string _urlApi = "http://localhost:5247/ReseñasClientes";

        // El constructor recibe el puente mágicamente
        public ReseñasClientesService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<ReseñasClientesClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<ReseñasClientesClase>>(json!) ?? new List<ReseñasClientesClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<ReseñasClientesClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /ReseñasClientes/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<ReseñasClientesClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(ReseñasClientesClase ReseñaCliente)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", ReseñaCliente } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(ReseñasClientesClase ReseñaCliente)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", ReseñaCliente } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
