...
namespace PresentacionesAspnetcoreBarberia.Services
{
    public class ReservasService
    {
        ...
        private readonly string _urlApi = "http://localhost:5247/Reservas";

        // El constructor recibe el puente mágicamente
        public ReservasService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<ReservasClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<ReservasClase>>(json!) ?? new List<ReservasClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<ReservasClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /Reservas/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<ReservasClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(ReservasClase Reserva)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", Reserva } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(ReservasClase Reserva)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", Reserva } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
