...
namespace PresentacionesAspnetcoreBarberia.Services
{
    public class FacturasService
    {
        ...
        private readonly string _urlApi = "http://localhost:5247/Facturas";

        // El constructor recibe el puente mágicamente
        public FacturasService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<FacturasClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<FacturasClase>>(json!) ?? new List<FacturasClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<FacturasClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /Facturas/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<FacturasClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(FacturasClase Factura)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", Factura } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(FacturasClase Factura)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", Factura } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
