...
namespace PresentacionesAspnetcoreBarberia.Services
{
    public class PortafoliosService
    {
        ...
        private readonly string _urlApi = "http://localhost:5247/Portafolios";

        // El constructor recibe el puente mágicamente
        public PortafoliosService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<PortafoliosClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<PortafoliosClase>>(json!) ?? new List<PortafoliosClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<PortafoliosClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /Portafolios/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<PortafoliosClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(PortafoliosClase Portafolio)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", Portafolio } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(PortafoliosClase Portafolio)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", Portafolio } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
