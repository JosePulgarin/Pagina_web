...
{
    public class PerfilUsuariosService
    {
        private readonly IComunicaciones _puente;

        // La URL base de tu API (Asegúrate de poner el puerto correcto que te da Swagger)
        private readonly string _urlApi = "http://localhost:5247/PerfilUsuarios";

        // El constructor recibe el puente mágicamente
        public PerfilUsuariosService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<PerfilUsuariosClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<PerfilUsuariosClase>>(json!) ?? new List<PerfilUsuariosClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<PerfilUsuariosClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /PerfilUsuarios/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<PerfilUsuariosClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(PerfilUsuariosClase PerfilUsuario)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", PerfilUsuario } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(PerfilUsuariosClase PerfilUsuario)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", PerfilUsuario } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
