...
namespace PresentacionesAspnetcoreBarberia.Services
{
    public class SedesService
    {
        ...
        private readonly string _urlApi = "http://localhost:5247/Sedes";

        // El constructor recibe el puente mágicamente
        public SedesService(IComunicaciones puente)
        {
            _puente = puente;
        }

        // 1. CONSULTAR
        public async Task<List<SedesClase>> ConsultarAsync()
        {
            ...
            return JsonConvert.DeserializeObject<List<SedesClase>>(json!) ?? new List<SedesClase>();
        }

        // 2. CONSULTAR POR ID (Para llenar los datos en la pantalla Edit)
        public async Task<SedesClase?> ConsultarPorIdAsync(int id)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", $"{_urlApi}/{id}" }, // Le pegamos el ID a la URL (Ej: /Sedes/5)
                { "Tipo", "GET" }
            };
            ...
            return JsonConvert.DeserializeObject<SedesClase>(json);
        }


        // 3. GUARDAR
        public async Task<bool> GuardarAsync(SedesClase Sede)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "POST" },
                { "Entidad", Sede } // Le pasamos los datos para que el puente arme el paquete
            };
        ...
        public async Task<bool> ModificarAsync(SedesClase Sede)
        {
            var peticion = new Dictionary<string, object>
            {
                { "Url", _urlApi },
                { "Tipo", "PUT" },
                { "Entidad", Sede } // Mandamos el objeto entero (que ya trae su ID adentro)
            };
