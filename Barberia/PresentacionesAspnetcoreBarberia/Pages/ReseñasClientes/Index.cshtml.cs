...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _ReseñasClientesService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("ReseñasClientes");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase ReseñasClientes)
                hoja.Cell(1, 1).Value = "ID ReseñaCliente";
                hoja.Cell(1, 2).Value = "Puntuación";
                hoja.Cell(1, 3).Value = "Comentario";
                hoja.Cell(1, 4).Value = "Fecha publicación";
                hoja.Cell(1, 5).Value = "Etiquetas";
                hoja.Cell(1, 6).Value = "Id reserva";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 2).Value = item.Puntuacion;
                    hoja.Cell(fila, 3).Value = item.Comentario;
                    hoja.Cell(fila, 4).Value = item.FechaPublicacion.ToDateTime(TimeOnly.MinValue);
                    hoja.Cell(fila, 5).Value = item.Etiquetas;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_ReseñasClientes.xlsx");
                }
            }
        }
=======
