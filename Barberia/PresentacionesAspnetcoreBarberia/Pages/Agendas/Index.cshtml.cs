...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _AgendasService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("Reporte de Agendas");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase Agendas)
                hoja.Cell(1, 1).Value = "ID Agenda";
                hoja.Cell(1, 2).Value = "Fecha";
                hoja.Cell(1, 3).Value = "Hora";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 2).Value = item.Fecha.ToDateTime(TimeOnly.MinValue);
                    hoja.Cell(fila, 3).Value = item.Hora.ToTimeSpan();
                    hoja.Cell(fila, 4).Value = item.Estado;
                    hoja.Cell(fila, 5).Value = item.IdBarbero;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_Agendas.xlsx");
                }
            }
        }
