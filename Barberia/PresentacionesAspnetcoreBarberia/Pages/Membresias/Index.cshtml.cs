...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _MembresiasService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("Reporte de Membresias");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase Membresias)
                hoja.Cell(1, 1).Value = "ID Membresia";
                hoja.Cell(1, 2).Value = "Nombre plan";
                hoja.Cell(1, 3).Value = "Costo mensual";
                hoja.Cell(1, 4).Value = "Descuento porcentaje";
                hoja.Cell(1, 5).Value = "Dias de vigencia";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 2).Value = item.NombrePlan;
                    hoja.Cell(fila, 3).Value = item.CostoMensual;
                    hoja.Cell(fila, 5).Value = item.DescuentoPorcentaje;
                    hoja.Cell(fila, 6).Value = item.DiaVigencia;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_Membresias.xlsx");
                }
            }
        }
=======
