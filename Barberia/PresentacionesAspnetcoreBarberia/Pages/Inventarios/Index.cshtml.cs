...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _InventariosService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("Reporte de Inventarios");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase Inventarios)
                hoja.Cell(1, 1).Value = "ID Inventario";
                hoja.Cell(1, 2).Value = "Nombre";
                hoja.Cell(1, 3).Value = "Descripción";
                hoja.Cell(1, 4).Value = "Cantidad";
                hoja.Cell(1, 5).Value = "Fecha abastecimiento";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 4).Value = item.CantidadActual;
                    hoja.Cell(fila, 5).Value = item.FechaAbastecimiento.ToDateTime(TimeOnly.MinValue);    
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_Inventarios.xlsx");
                }
            }
        }
=======
