...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _GastosOperativosService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("GastosOperativos");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase GastosOperativos)
                hoja.Cell(1, 1).Value = "ID GastoOperativo";
                hoja.Cell(1, 2).Value = "Categoria";
                hoja.Cell(1, 3).Value = "Monto";
                hoja.Cell(1, 4).Value = "Fecha Pago";
                hoja.Cell(1, 5).Value = "Numero comprobante";
                hoja.Cell(1, 6).Value = "ID sede";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 4).Value = item.FechaPago.ToDateTime(TimeOnly.MinValue);
                    hoja.Cell(fila, 5).Value = item.NumeroComprobante;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_GastosOperativos.xlsx");
                }
            }
        }
=======
