...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _PerfilUsuariosService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("PerfilUsuarios");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase PerfilUsuarios)
                hoja.Cell(1, 1).Value = "ID PerfilUsuario";
                hoja.Cell(1, 3).Value = "Correo";
                hoja.Cell(1, 4).Value = "Contraseña";
                hoja.Cell(1, 5).Value = "Estado";
                hoja.Cell(1, 8).Value = "ID rol";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 5).Value = item.Contraseña;
                    hoja.Cell(fila, 6).Value = item.Estado;
                    hoja.Cell(fila, 8).Value = item.IdRol;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_PerfilUsuarios.xlsx");
                }
            }
        }
=======
