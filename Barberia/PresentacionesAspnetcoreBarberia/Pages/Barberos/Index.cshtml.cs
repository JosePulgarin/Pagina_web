...
namespace PresentacionesAspnetcoreBarberia.Pages.Barberos
{
    public class IndexModel : PageModel
    {
        ...
        // Usamos nuestra nueva "maleta" de BarberoModel
        public List<BarberosClase> ListaBarberos { get; set; } = new List<BarberosClase>();        
        // Inyectamos el servicio moderno
        public IndexModel(BarberosService BarberosService, HistoricosService historicosService)
        {
        ...
        {
            // Llama a tu servicio para que borre el Barbero con ese ID
            // NOTA: Asegúrate de tener este método EliminarAsync creado en tu BarberosService
            var exito = await _BarberosService.EliminarAsync(id);
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _BarberosService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("Reporte de Barberos");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase Barberos)
                hoja.Cell(1, 1).Value = "ID Barbero";
                hoja.Cell(1, 2).Value = "Nombre";
                hoja.Cell(1, 3).Value = "Correo";
                hoja.Cell(1, 4).Value = "Fecha nacimiento";
                hoja.Cell(1, 5).Value = "Especialidad";
                hoja.Cell(1, 6).Value = "Biografia";
                hoja.Cell(1, 7).Value = "ID usuario";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 4).Value = item.FechaNacimiento.ToDateTime(TimeOnly.MinValue);
                    hoja.Cell(fila, 5).Value = item.Especialidad;
                    hoja.Cell(fila, 6).Value = item.Biografia;
                    hoja.Cell(fila, 7).Value = item.IdUsuario;
                    hoja.Cell(fila, 8).Value = item.IdSede;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_Barberos.xlsx");
                }
            }
        }
    }
