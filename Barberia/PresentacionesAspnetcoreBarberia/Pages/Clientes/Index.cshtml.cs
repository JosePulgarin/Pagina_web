...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _ClientesService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("Reporte de Clientes");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase Clientes)
                hoja.Cell(1, 1).Value = "ID Cliente";
                hoja.Cell(1, 2).Value = "Nombre";
                hoja.Cell(1, 3).Value = "Telefono";
                hoja.Cell(1, 4).Value = "Correo";
                hoja.Cell(1, 5).Value = "ID usuario";
                hoja.Cell(1, 6).Value = "ID sede";
                hoja.Cell(1, 7).Value = "ID Membresia";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 3).Value = item.Telefono;
                    hoja.Cell(fila, 4).Value = item.Correo;
                    hoja.Cell(fila, 5).Value = item.IdUsuario;
                    hoja.Cell(fila, 6).Value = item.IdSede;
                    hoja.Cell(fila, 7).Value = item.IdMembresia;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_Clientes.xlsx");
                }
            }
        }
=======
