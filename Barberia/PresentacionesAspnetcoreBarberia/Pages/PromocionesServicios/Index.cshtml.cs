...
namespace PresentacionesAspnetcoreBarberia.Pages.PromocionesServicios
{
    public class IndexModel : PageModel
    {
        ...
        // Usamos nuestra nueva "maleta" de PromocionServicioModel
        public List<PromocionesServiciosClase> ListaPromocionesServicios { get; set; } = new List<PromocionesServiciosClase>();        
        // Inyectamos el servicio moderno
        public IndexModel(PromocionesServiciosService PromocionesServiciosService, HistoricosService historicosService)
        {
...
>>>>>>> 112295c9af2e932238842f9f0beda5235c10b042
            ...
            ListaPromocionesServicios = await _PromocionesServiciosService.ConsultarAsync();
...
>>>>>>> 112295c9af2e932238842f9f0beda5235c10b042
                Accion = "Consultó la lista de PromocionesServicios",
                Fecha = DateTime.Now
            };
...
>>>>>>> 112295c9af2e932238842f9f0beda5235c10b042
        ...
        {
            // Llama a tu servicio para que borre el PromocionServicio con ese ID
            // NOTA: Asegúrate de tener este método EliminarAsync creado en tu PromocionesServiciosService
            var exito = await _PromocionesServiciosService.EliminarAsync(id);
...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _PromocionesServiciosService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("PromocionesServicios");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase PromocionesServicios)
                hoja.Cell(1, 1).Value = "ID PromocionServicio";
                hoja.Cell(1, 2).Value = "Monto descuento";
                hoja.Cell(1, 3).Value = "Descuento finde";
                hoja.Cell(1, 4).Value = "Id Servicio";
                hoja.Cell(1, 5).Value = "Id promocion especial";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 2).Value = item.MontoDescuento;
                    hoja.Cell(fila, 3).Value = item.DescuentoFinde;
                    hoja.Cell(fila, 5).Value = item.IdServicio;
                    hoja.Cell(fila, 6).Value = item.IdPromocionEspecial;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_PromocionesServicios.xlsx");
                }
            }
        }
=======
