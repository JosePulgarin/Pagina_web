...
<<<<<<< HEAD
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync()
        {
            ...
            var listaDatos = await _ProveedoresService.ConsultarAsync();

            // 2. Creamos el archivo de Excel virtual
            using (var workbook = new XLWorkbook())
            {
                // Nombre de la pestaña abajo en el Excel
                var hoja = workbook.Worksheets.Add("Reporte de Proveedores");

                // 3. Pintamos los títulos de las columnas (Fila 1)
                // (Ajuste "ColumnaX" por los nombres reales de los atributos de su clase Proveedores)
                hoja.Cell(1, 1).Value = "ID Proveedor";
                hoja.Cell(1, 2).Value = "Nombre";
                hoja.Cell(1, 3).Value = "Nombre empresa";
                ...
                foreach (var item in listaDatos)
                {
                    ...
                    hoja.Cell(fila, 3).Value = item.NombreEmpresa;
                    hoja.Cell(fila, 5).Value = item.Correo;
                    hoja.Cell(fila, 6).Value = item.Telefono;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(contenido, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Reporte_Proveedores.xlsx");
                }
            }
        }
=======
